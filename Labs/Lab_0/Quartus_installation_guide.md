...
Para poder ejecutar **Quartus** desde cualquier ubicación en la terminal, es necesario agregar su ruta de instalación a la variable de entorno `PATH`. Esto le indica al sistema operativo dónde encontrar los ejecutables del software y simplifica el acceso a la herramienta.

Esta configuración se realiza a través del archivo **`.bashrc`**, el cual se ejecuta automáticamente cada vez que se abre una nueva sesión de terminal. Dicho archivo se encuentra en el directorio *home* del usuario y se utiliza para definir variables de entorno y personalizar el comportamiento de la terminal.

Para agregar la configuración, ejecute el siguiente comando en la terminal:
...
EOF
``` 
Este comando añade automáticamente las variables necesarias al archivo .bashrc, utilizando la ruta del directorio home del usuario.

Asegúrese de que el nombre del directorio (intelFPGA_lite/23.1std) coincida con la versión de Quartus instalada en su sistema.

Para aplicar los cambios sin reiniciar la terminal, ejecute:
```bash
source ~/.bashrc
```

A continuación, cree un enlace simbólico al ejecutable de Quartus para facilitar su ejecución desde cualquier ubicación en la terminal:
```bash
sudo ln -s $QUARTUS_ROOTDIR/bin/quartus /bin/quartus
```

Una vez finalizado este proceso, podrá ejecutar la IDE de Quartus escribiendo en la terminal:
```bash
quartus
```
